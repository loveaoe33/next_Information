...
export async function DELETE(request: Request) { //delet data
  ...
  try {
    ...
    const { id, hashcode, domainUrl, header, father_header, userData }: RequestBody = body;
    const response = await fetch(`${domainUrl}/Product_Imformation/deleteProduct_Information?caseSelect=midCase`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: "Kid",
...
export async function PATCH(request: Request) {   //update state
  ...
  try {
    ...
    const response = await fetch(`${domainUrl}/Product_Imformation/updateProduct_State?caseSelect=midCase`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: "Kid",
